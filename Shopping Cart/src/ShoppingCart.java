		...
		try (Scanner scan = new Scanner(System.in)) {
			while (true) {
				...
				try {
					...
					switch (choice) {
					case 1:
						i.displayList(Item.itemsList);
						System.out.print("Enter Item id to add item to Cart : ");
						String addItemid = scan.next();
						Item addItem = m.findItem(Item.itemsList, addItemid); // Function to find an item in the List of
																				// stored item using item's unique id
						if (addItem != null) {
					...
					case 2:
						m.showCart(); // Display all items which are added to the cart
						break;
					...
					System.out.print("Illegal input ! Please try again ");// Invalid input exception handling using
																			// try-catch
				}
			}
		}
